...
class RBCConfig:
    ...
    hidden: int = 192
    lstm_layers: int = 2
    layers: int = 2  # kept only for metadata compatibility; not used by LSTM backbone
    ...
    out_dir: str = "./outputs/RBC/LSTM-PINN"
    seed: int = 0
...
def batch_losses(cfg: RBCConfig, model: nn.Module, col, ic, bc_left, bc_right, bc_bottom, bc_top):
    with torch.enable_grad():
...
@torch.no_grad()
def data_loss_fields(cfg: RBCConfig, model: nn.Module, batch) -> Dict[str, float]:
    x, y, t = batch
...
@torch.no_grad()
def evaluate_val_metrics(cfg: RBCConfig, model: nn.Module, data, n: int = 3000) -> Dict[str, float]:
...
@torch.no_grad()
def eval_on_grid(cfg: RBCConfig, model: nn.Module):
...
def train(cfg: RBCConfig):
    ...
    model = LSTMPINNModel(cfg).to(cfg.device, cfg.dtype)
