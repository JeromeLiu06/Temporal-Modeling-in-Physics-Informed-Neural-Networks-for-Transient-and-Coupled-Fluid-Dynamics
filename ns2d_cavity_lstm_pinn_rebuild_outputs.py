...
class CavityConfig:
    ...
    # LSTM-PINN architecture. Kept aligned with the original Lid LSTM-PINN.
    hidden: int = 192
    lstm_layers: int = 2
    ...
    # Output
    out_dir: str = "./outputs/Lid/LSTM-PINN"
    seed: int = 0
...
class BaselineModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        seq = torch.stack([x, y], dim=1)  # [B, 2, 1]
        seq = self.embed(seq)            # [B, 2, H]
...
def save_model_info(cfg: CavityConfig, model: nn.Module):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: LSTM-PINN with coordinate-sequence input\n")
        f.write("Input dimension: 2 (x, y)\n")
        f.write("Sequence length: 2\n")
        f.write("Output dimension: 3 (u, v, p)\n")
        f.write(f"LSTM hidden dimension: {cfg.hidden}\n")
        f.write(f"LSTM layers: {cfg.lstm_layers}\n")
        f.write("Head: Linear(hidden, hidden) + Tanh + Linear(hidden, 3)\n")
        f.write("Activation: Tanh in prediction head\n")
